...
import static fr.insa.zins.classe.Etudiant.trouveEtudiantListe;
...
public class ModifEtudiantSupp extends FormLayout {
    ...
    public ModifEtudiantSupp(VuePrincipale main) {
        ...
        this.enregistrer.addClickListener(event -> { 
                 try{
                 /*if (trouveIdEtudiant(this.main.getConBdD(), this.nom.getValue())==-1){
                     Notification.show("L'étudiant n'existe pas");
                 }else{*/
                 Etudiant.deleteEtudiant(this.main.getConBdD(), this.nom.getValue());
                getListeEtudiant().remove(trouveEtudiantListe(this.main.getConBdD(), this.nom.getValue()));
